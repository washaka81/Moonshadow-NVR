...
epochs = 20
learning_rate = 0.001
lpr_max_len = 8  # max plate length
...
# Freeze some layers? Let's freeze backbone and train only container and final layers
# for name, param in model.named_parameters():
#     if 'container' not in name and 'backbone.20' not in name and 'backbone.21' not in name:
#         param.requires_grad = False
# print("Frozen backbone, training only final layers")
...
# Loss and optimizer
criterion = nn.CTCLoss(blank=num_classes-1, zero_infinity=True)  # blank is last index (dash)
...
def train_epoch(model, device, train_loader, criterion, optimizer, epoch):
    ...
    for batch_idx, (images, targets, labels) in enumerate(train_loader):
        ...
        # CTC loss expects input: (seq_len, batch, num_classes)
        # model output: (batch, num_classes, seq_len)
        outputs = model(images)  # (batch, num_classes, seq_len)
        outputs = outputs.permute(2, 0, 1)  # (seq_len, batch, num_classes)
        # Target lengths
        target_lengths = torch.tensor([len(t) for t in targets], dtype=torch.long)
        # Input lengths (same for all samples)
