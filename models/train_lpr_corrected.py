...
def train_epoch(model, device, train_loader, criterion, optimizer, epoch, blank_index):
    ...
    for batch_idx, (images, indices, target_lens, labels) in enumerate(train_loader):
        ...
        # Flatten targets (indices are padded with blank, need to remove padding)
        targets_flat = []
        for i in range(indices.size(0)):
            # indices[i] shape (lpr_max_len), target_lens[i] is actual length
...
def main():
    ...
    for epoch in range(1, epochs+1):
        ...
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save(model.state_dict(), f'chilean_lpr_best_corrected.pth')
            print(f'Best model saved with accuracy {val_acc:.4f}')
    torch.save(model.state_dict(), f'chilean_lpr_final_corrected.pth')
