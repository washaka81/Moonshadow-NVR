...
function renderObjectType(event: EnhancedAiEvent) {
  ...
  return (
    <Box sx={{ display: "flex", alignItems: "center", gap: 1 }}>
      <Box
        sx={{ color: `${getColorForType(event.type_)}.main`, display: "flex" }}
      >
  ...
  const fetchEvents = useCallback(async () => {
    ...
    try {
      ...
      if (resp.status === "success") {
        ...
        setError(
          `Error: ${resp.status === "error" ? resp.message : resp.status}`,
        );
      }
    } catch (e) {
      setLoading(false);
                  ...
                  {events.length > 0 ? (
                    events.map((event, idx) => (
                      ...
                      >
                        ...
                        <TableCell>{renderObjectType(event)}</TableCell>
                        <TableCell>
                  ...
                  {events.map((event, idx) => (
                    <TimelineItem key={idx}>
                      <TimelineOppositeContent
                        sx={{ flex: "0.1", textAlign: "right", pt: 2 }}
                      >
                        <Typography variant="body2" sx={{ fontWeight: 600 }}>
                          {event.formattedDate}
                        </Typography>
                      ...
                      <TimelineSeparator>
                        <TimelineDot
                          sx={{
                            bgcolor: `${getColorForType(event.type_)}.main`,
                            mt: 2,
                          }}
                        >
